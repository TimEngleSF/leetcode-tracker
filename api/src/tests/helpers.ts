...
export const createMockDb = async (): Promise<MockDb> => {
    try {
        ...
        try {
            const questionFileContents = fs.readFileSync(
                questionsPath,
                'utf-8'
            );
        ...
        if ((await db.collection('questionData').countDocuments()) === 0) {
            await db
                ...
                .insertMany(questionInfoDocuments);
        }

        // Create indexes
        await userCollection.createIndex({ email: 1 });
        await questionCollection.createIndexes([
            { key: { created: 1 } },
            { key: { passed: 1 } },
            { key: { questNum: 1 } },
            { key: { userId: 1 } }
        ]);
        await questionInfoCollection.createIndex(
            { questId: 1 },
            { unique: true }
        );
        await groupCollection.createIndex({ name: 1 }, { unique: true });

        const MockDb = { db, client, uri, mongoServer };
        return MockDb;
    } catch (error: any) {
        throw new Error(`Cannot connect to mock db: ${error.message}`);
    }
};
